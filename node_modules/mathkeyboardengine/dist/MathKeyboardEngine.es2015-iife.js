...
var mke = (() => {
  ...
  var x_exports = {};
  __export(x_exports, {
    AscendingBranchingNode: () => AscendingBranchingNode,
    BranchingNode: () => BranchingNode,
    DecimalSeparatorNode: () => DecimalSeparatorNode,
    DescendingBranchingNode: () => DescendingBranchingNode,
    DigitNode: () => DigitNode,
    ...
    RoundBracketsNode: () => RoundBracketsNode,
    StandardBranchingNode: () => StandardBranchingNode,
    StandardLeafNode: () => StandardLeafNode,
    TreeNode: () => TreeNode,
    ...
    insert: () => insert,
    insertWithEncapsulateCurrent: () => insertWithEncapsulateCurrent,
    insertWithEncapsulateSelection: () => insertWithEncapsulateSelection,
    insertWithEncapsulateSelectionAndPrevious: () => insertWithEncapsulateSelectionAndPrevious,
  ...
  function getViewModeLatex(x, latexConfiguration) {
    const syntaxTreeComponent = x instanceof KeyboardMemory ? x.syntaxTreeRoot : x;
  ...
  function encapsulateAllPartsOfNumberWithDigitsLeftOfIndex(exclusiveRightIndex, siblingNodes, toPlaceholder) {
  ...
  function deleteLeft(k) {
    ...
    if (k.current instanceof Placeholder) {
      if (k.current.parentNode == null || k.current.nodes.length > 0) {
        ...
        const nonEmptyPlaceholderOnLeft = getFirstNonEmptyOnLeftOf(k.current.parentNode.placeholders, k.current);
        if (nonEmptyPlaceholderOnLeft) {
          if (k.current.parentNode.placeholders.length == 2 && k.current === k.current.parentNode.placeholders[1] && k.current.nodes.length == 0) {
            ...
            k.current = (_a = lastOrNull(nonEmptyPlaceholderOnLeft.nodes)) != null ? _a : nonEmptyPlaceholderOnLeft;
          }
          ...
          k.current = previousNode != null ? previousNode : ancestorPlaceholder;
      ...
      if (k.current instanceof BranchingNode && k.current.placeholders[0].nodes.length > 0 && k.current.placeholders.slice(1).every((ph) => ph.nodes.length == 0)) {
        ...
        const previousNode = firstBeforeOrNull(k.current.parentPlaceholder.nodes, k.current);
        remove(k.current.parentPlaceholder.nodes, k.current);
        k.current = previousNode != null ? previousNode : k.current.parentPlaceholder;
      }
    }
  }
  ...
  function deleteRight(k) {
    ...
    if (k.current instanceof Placeholder) {
      if (k.current.parentNode != null && k.current.parentNode.placeholders.every((ph) => ph.nodes.length == 0)) {
        ...
        k.current = previousNode != null ? previousNode : k.current.parentNode.parentPlaceholder;
      } else {
        ...
        if (nodes.length > 0) {
          ...
          if (siblingPlaceholders[0] == k.current && siblingPlaceholders.length == 2) {
            ...
            k.current = (_a = firstBeforeOrNull(parentNode.parentPlaceholder.nodes, parentNode)) != null ? _a : parentNode.parentPlaceholder;
            deleteOuterBranchingNodeButNotItsContents(nonEmptyPlaceholder);
          } else {
  ...
  function popSelection(k) {
    ...
    if (k.current instanceof Placeholder) {
      ...
      k.current = (_a = firstBeforeOrNull(siblings, k.inclusiveSelectionLeftBorder)) != null ? _a : k.current.parentPlaceholder;
      leaveSelectionMode(k);
  ...
  function moveRight(k) {
    ...
    if (k.current instanceof Placeholder) {
      if (k.current.nodes.length > 0) {
        ...
        k.current = (_a = firstAfterOrNull(k.current.parentNode.placeholders, k.current)) != null ? _a : k.current.parentNode;
      }
    } else {
      const nextNode = firstAfterOrNull(k.current.parentPlaceholder.nodes, k.current);
      if (nextNode != null) {
        ...
        if (ancestorNode != null) {
          const nextPlaceholder = firstAfterOrNull(ancestorNode.placeholders, k.current.parentPlaceholder);
          k.current = nextPlaceholder != null ? nextPlaceholder : ancestorNode;
        }
      }
    }
  }
  ...
  function insertWithEncapsulateCurrent(k, newNode, config) {
    ...
    if (k.current instanceof TreeNode) {
      ...
      if (k.current instanceof RoundBracketsNode && (config == null ? void 0 : config.deleteOuterRoundBracketsIfAny)) {
        encapsulate(k.current.placeholders[0].nodes, encapsulatingPlaceholder);
        k.current = (_a = firstAfterOrNull(newNode.placeholders, encapsulatingPlaceholder)) != null ? _a : newNode;
  ...
  function insertWithEncapsulateSelectionAndPrevious(k, newNode) {
    ...
    k.current = (_a = lastOrNull(selection)) != null ? _a : secondPlaceholder;
  }
  ...
  function moveDown(k) {
    ...
    while (true) {
      ...
      if (suggestion != null) {
        k.current = (_a = lastOrNull(suggestion.nodes)) != null ? _a : suggestion;
        return;
      }
  ...
  function moveLeft(k) {
    ...
    if (k.current instanceof Placeholder) {
      ...
      const previousPlaceholder = firstBeforeOrNull(k.current.parentNode.placeholders, k.current);
      if (previousPlaceholder !== null) {
        k.current = (_a = lastOrNull(previousPlaceholder.nodes)) != null ? _a : previousPlaceholder;
      } else {
        const ancestorPlaceholder = k.current.parentNode.parentPlaceholder;
        const nodePreviousToParentOfCurrent = firstBeforeOrNull(ancestorPlaceholder.nodes, k.current.parentNode);
        k.current = nodePreviousToParentOfCurrent != null ? nodePreviousToParentOfCurrent : ancestorPlaceholder;
      }
    } else {
      if (k.current instanceof BranchingNode) {
        const placeholder = last(k.current.placeholders);
        k.current = (_b = lastOrNull(placeholder.nodes)) != null ? _b : placeholder;
      } else {
        k.current = (_c = firstBeforeOrNull(k.current.parentPlaceholder.nodes, k.current)) != null ? _c : k.current.parentPlaceholder;
      }
    }
  }
  ...
  function selectLeft(k) {
    var _a;
    const oldDiffWithCurrent = (_a = k.selectionDiff) != null ? _a : 0;
    ...
    } else if (k.inclusiveSelectionLeftBorder instanceof TreeNode && k.inclusiveSelectionLeftBorder.parentPlaceholder.nodes.indexOf(k.inclusiveSelectionLeftBorder) == 0 && k.inclusiveSelectionLeftBorder.parentPlaceholder.parentNode != null) {
  ...
  function selectRight(k) {
    var _a, _b;
    const oldDiffWithCurrent = (_a = k.selectionDiff) != null ? _a : 0;
    if (k.current instanceof Placeholder && oldDiffWithCurrent < k.current.nodes.length || k.current instanceof TreeNode && k.current.parentPlaceholder.nodes.indexOf(k.current) + oldDiffWithCurrent < k.current.parentPlaceholder.nodes.length - 1) {
      setSelectionDiff(k, oldDiffWithCurrent + 1);
    } else if (k.inclusiveSelectionRightBorder instanceof TreeNode && last(k.inclusiveSelectionRightBorder.parentPlaceholder.nodes) == k.inclusiveSelectionRightBorder && k.inclusiveSelectionRightBorder.parentPlaceholder.parentNode != null) {
      const ancestorNode = k.inclusiveSelectionRightBorder.parentPlaceholder.parentNode;
      k.current = (_b = firstBeforeOrNull(ancestorNode.parentPlaceholder.nodes, ancestorNode)) != null ? _b : ancestorNode.parentPlaceholder;
      setSelectionDiff(k, 1);
    }
  }
  ...
  function parseLatex(latex, latexParserConfiguration = new LatexParserConfiguration()) {
    var _a, _b;
    if (latex == null) {
    ...
    while (x != "") {
      ...
      if (decimalSeparatorMatch != null) {
        insert(k, new DecimalSeparatorNode((_a = latexParserConfiguration.preferredDecimalSeparator) != null ? _a : decimalSeparatorMatch));
        x = x.slice(decimalSeparatorMatch.length);
        continue;
      }
      if (["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"].includes(x[0]) || ((_b = latexParserConfiguration.additionalDigits) == null ? void 0 : _b.includes(x[0]))) {
      ...
      if (x.startsWith(String.raw`\begin{`)) {
        ...
        for (const line of lines) {
          for (const latex2 of line.split("&")) {
            const nodes = parseLatex(latex2, latexParserConfiguration).syntaxTreeRoot.nodes;
            insert(k, nodes);
            moveRight(k);
          }
        }
